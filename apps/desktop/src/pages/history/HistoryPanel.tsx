...
import type { ChatHistoryEntry } from '../../lib/db/db';
...
export function HistoryPanel({
  ...
  const threadMap = Object.fromEntries(threads.map((thread) => [thread.id, thread.name || thread.preview || thread.id]));
        ...
        {items.map((entry) => (
          <div key={entry.id} className="history-card" onClick={() => onSelectMessage(entry.message)}>
            ...
            <div className="history-card-info">
              <div className="history-card-msg">
        ...
        <div className="providers-list history-list">
          {entries.length === 0 ? (
            <DesktopEmptyState
              ...
              description="Saved prompts and reusable conversation starters will appear here."
            />
          ) : (
