...
fn run_migrations(conn: &Connection) -> Result<(), String> {
    ...
    for (col, def) in [
        ...
        let _ = conn.execute(
            &format!("ALTER TABLE automations ADD COLUMN {col} {def}"),
            [],
        );
    }
...
fn push_int(
    ...
    match v {
        serde_json::Value::Number(n) => {
            sets.push(format!("{col} = ?{idx}"));
            vals.push(Value::Integer(n.as_i64().unwrap_or(0)));
            *idx += 1;
        }
        serde_json::Value::Bool(b) => {
            sets.push(format!("{col} = ?{idx}"));
            vals.push(Value::Integer(if *b { 1 } else { 0 }));
            *idx += 1;
        }
        serde_json::Value::Null => {
            sets.push(format!("{col} = ?{idx}"));
...
pub fn db_delete_chat_config(thread_id: String) -> Result<(), String> {
    ...
    let conn = guard.as_ref().ok_or("DB not initialized")?;
    conn.execute(
        "DELETE FROM chat_config WHERE thread_id = ?1",
        params![thread_id],
    )
...
#[tauri::command]
pub fn db_search_chat_history(
    query: String,
    limit: Option<i64>,
) -> Result<Vec<ChatHistoryRow>, String> {
    let guard = get_conn()?;
    let conn = guard.as_ref().ok_or("DB not initialized")?;
    let lim = limit.unwrap_or(200);
...
pub fn db_set_default_connection(id: String) -> Result<(), String> {
    ...
    conn.execute("UPDATE saved_connections SET is_default = 0", [])
        .map_err(|e| format!("Reset: {e}"))?;
    conn.execute(
        "UPDATE saved_connections SET is_default = 1 WHERE id = ?1",
        params![id],
    )
...
fn read_automation_row(r: &rusqlite::Row) -> rusqlite::Result<AutomationRow> {
    Ok(AutomationRow {
        ...
        prompt: r.get(2)?,
        project_cwd: r.get(3)?,
        retry_enabled: r.get(4)?,
        retry_max_attempts: r.get(5)?,
        retry_backoff_minutes: r.get(6)?,
        retry_count: r.get(7)?,
        pending_run_kind: r.get(8)?,
        status: r.get(9)?,
        schedule_mode: r.get(10)?,
        schedule_weekdays: r.get(11)?,
        schedule_time: r.get(12)?,
        schedule_interval_hours: r.get(13)?,
        schedule_custom_rrule: r.get(14)?,
        last_run_at: r.get(15)?,
        next_run_at: r.get(16)?,
        next_scheduled_run_at: r.get(17)?,
        last_thread_id: r.get(18)?,
        last_run_status: r.get(19)?,
        last_error: r.get(20)?,
        background_notify: r.get(21)?,
        template_id: r.get(22)?,
        created_at: r.get(23)?,
        updated_at: r.get(24)?,
    })
}
...
pub fn db_create_automation(
    ...
    retry_backoff_minutes: Option<i64>,
    background_notify: Option<bool>,
    status: Option<String>,
    schedule_mode: String,
    schedule_weekdays: String,
    schedule_time: String,
    schedule_interval_hours: Option<i64>,
    schedule_custom_rrule: Option<String>,
    template_id: Option<String>,
) -> Result<(), String> {
    let guard = get_conn()?;
    let conn = guard.as_ref().ok_or("DB not initialized")?;
    let re: i64 = if retry_enabled.unwrap_or(true) { 1 } else { 0 };
    let bn: i64 = if background_notify.unwrap_or(true) {
        1
    } else {
        0
    };
...
pub fn db_update_automation(updates_json: String) -> Result<(), String> {
    let obj: serde_json::Value =
        serde_json::from_str(&updates_json).map_err(|e| format!("Parse: {e}"))?;
    ...
    let text_cols = [
        ...
        ("pendingRunKind", "pending_run_kind"),
        ("status", "status"),
        ("scheduleMode", "schedule_mode"),
        ("scheduleWeekdays", "schedule_weekdays"),
        ("scheduleTime", "schedule_time"),
        ("scheduleCustomRrule", "schedule_custom_rrule"),
        ("lastThreadId", "last_thread_id"),
        ("lastRunStatus", "last_run_status"),
        ("lastError", "last_error"),
    ];
    for (jk, col) in text_cols {
        if let Some(v) = map.get(jk) {
            push_text(&mut sets, &mut vals, &mut idx, col, v);
        }
    }
    ...
    for (jk, col) in int_cols {
        if let Some(v) = map.get(jk) {
            push_int(&mut sets, &mut vals, &mut idx, col, v);
        }
    }
    ...
    let sql = format!(
        "UPDATE automations SET {} WHERE id = ?{idx}",
        sets.join(", ")
    );
...
fn read_run_row(r: &rusqlite::Row) -> rusqlite::Result<AutomationRunRow> {
    Ok(AutomationRunRow {
        ...
        trigger_source: r.get(3)?,
        status: r.get(4)?,
        attempt_number: r.get(5)?,
        started_at: r.get(6)?,
        finished_at: r.get(7)?,
        scheduled_for: r.get(8)?,
        retry_scheduled_for: r.get(9)?,
        thread_id: r.get(10)?,
...
#[tauri::command]
pub fn db_list_automation_runs(
    ...
    if let Some(aid) = automation_id {
        let mut stmt = conn
            ...
            .map_err(|e| format!("Prepare: {e}"))?;
        let rows = stmt
            .query_map(params![aid, lim], read_run_row)
            .map_err(|e| format!("Query: {e}"))?
            .filter_map(|r| r.ok())
            .collect();
        Ok(rows)
        ...
        let mut stmt = conn
            ...
            .map_err(|e| format!("Prepare: {e}"))?;
        let rows = stmt
            .query_map(params![lim], read_run_row)
            .map_err(|e| format!("Query: {e}"))?
            .filter_map(|r| r.ok())
            .collect();
        Ok(rows)
...
pub fn db_create_automation_run(
    ...
    retry_scheduled_for: Option<i64>,
...
pub fn db_update_automation_run(updates_json: String) -> Result<(), String> {
    ...
    if let Some(v) = map.get("status") {
        push_text(&mut sets, &mut vals, &mut idx, "status", v);
    }
    if let Some(v) = map.get("finishedAt") {
        push_int(&mut sets, &mut vals, &mut idx, "finished_at", v);
    }
    if let Some(v) = map.get("retryScheduledFor") {
        push_int(&mut sets, &mut vals, &mut idx, "retry_scheduled_for", v);
    }
    if let Some(v) = map.get("threadId") {
        push_text(&mut sets, &mut vals, &mut idx, "thread_id", v);
    }
    if let Some(v) = map.get("errorMessage") {
        push_text(&mut sets, &mut vals, &mut idx, "error_message", v);
    }
    ...
    vals.push(Value::Text(id.to_string()));
    let sql = format!(
        "UPDATE automation_runs SET {} WHERE id = ?{idx}",
        sets.join(", ")
    );
    conn.execute(&sql, rusqlite::params_from_iter(vals.iter()))
        .map_err(|e| format!("Update: {e}"))?;
...
pub fn db_update_claude_session(updates_json: String) -> Result<(), String> {
    ...
    if let Some(v) = map.get("title") {
        push_text(&mut sets, &mut vals, &mut idx, "title", v);
    }
    if let Some(v) = map.get("model") {
        push_text(&mut sets, &mut vals, &mut idx, "model", v);
    }
    if let Some(v) = map.get("isArchived") {
        push_int(&mut sets, &mut vals, &mut idx, "is_archived", v);
    }
    ...
    vals.push(Value::Text(id.to_string()));
    let sql = format!(
        "UPDATE claude_sessions SET {} WHERE id = ?{idx}",
        sets.join(", ")
    );
    conn.execute(&sql, rusqlite::params_from_iter(vals.iter()))
        .map_err(|e| format!("Update: {e}"))?;
...
pub fn compute_next_run(
    ...
    let allowed: std::collections::HashSet<u32> = weekdays
        .split(',')
        .filter_map(|d| day_idx(d.trim()))
        .collect();
    ...
    if target > now && allowed.contains(&wd) {
        return Ok(target);
    }
    for off in 1..=8u32 {
        if allowed.contains(&((wd + off) % 7)) {
            return Ok(target + (off as i64) * 86400);
        }
    }
    Ok(now + 86400)
}
...
pub fn db_get_usage_summary(
    ...
    if let Some(s) = start_date {
        conds.push(format!("created_at >= ?{}", d_params.len() + 1));
        d_params.push(Value::Integer(s));
    }
    if let Some(e) = end_date {
        conds.push(format!("created_at <= ?{}", d_params.len() + 1));
        d_params.push(Value::Integer(e));
    }
    if !conds.is_empty() {
        d_where = format!("WHERE {}", conds.join(" AND "));
    }
    ...
    let d_row = conn
        .query_row(&d_sql, rusqlite::params_from_iter(d_params.iter()), |r| {
            Ok((
                r.get::<_, i64>(0)?,
                r.get::<_, f64>(1)?,
                r.get::<_, i64>(2)?,
    ...
    if let Some(e) = end_date {
        ...
        r_params.push(Value::Integer(e));
    }
    if !r_conds.is_empty() {
        r_where = format!("WHERE {}", r_conds.join(" AND "));
    }
    ...
    let r_row = conn
        .query_row(&r_sql, rusqlite::params_from_iter(r_params.iter()), |r| {
            Ok((
                r.get::<_, i64>(0)?,
                r.get::<_, f64>(1)?,
                r.get::<_, i64>(2)?,
    ...
    let rate = if total_req > 0 {
        (success as f64 / total_req as f64) * 100.0
    } else {
        0.0
    };
...
pub fn db_get_usage_trends(
    ...
    let rows = stmt
        .query_map(params![start_ts, end_ts, bucket_secs], |r| {
            Ok((
                r.get::<_, i64>(0)?,
                r.get::<_, i64>(1)?,
                r.get::<_, f64>(2)?,
    ...
    for i in 0..bucket_count {
        ...
        if let Some(r) = bucket_map.get(&i) {
            stats.push(DailyStats {
                date,
                request_count: r.1,
                total_cost: format!("{:.6}", r.2),
                total_tokens: r.3,
                total_input_tokens: r.4,
                total_output_tokens: r.5,
                total_cache_creation_tokens: r.6,
                total_cache_read_tokens: r.7,
            });
        } else {
            stats.push(DailyStats {
                date,
                request_count: 0,
                total_cost: "0.000000".into(),
                total_tokens: 0,
...
fn read_log_row(r: &rusqlite::Row) -> rusqlite::Result<RequestLogRow> {
    Ok(RequestLogRow {
        ...
        input_cost_usd: r.get(11)?,
        output_cost_usd: r.get(12)?,
        cache_read_cost_usd: r.get(13)?,
        cache_creation_cost_usd: r.get(14)?,
        total_cost_usd: r.get(15)?,
        is_streaming: r.get::<_, i64>(16).unwrap_or(0) != 0,
        latency_ms: r.get(17)?,
        first_token_ms: r.get(18)?,
...
pub fn db_get_request_logs(
    ...
    if let Some(m) = obj.and_then(|o| o.get("appType")).and_then(|v| v.as_str()) {
        conds.push(format!("l.app_type = ?{idx}"));
        vals.push(Value::Text(m.into()));
        idx += 1;
    }
    if let Some(m) = obj
        .and_then(|o| o.get("providerName"))
        .and_then(|v| v.as_str())
    {
        conds.push(format!("p.name LIKE ?{idx}"));
        vals.push(Value::Text(format!("%{m}%")));
        idx += 1;
    }
    if let Some(m) = obj.and_then(|o| o.get("model")).and_then(|v| v.as_str()) {
        conds.push(format!("l.model LIKE ?{idx}"));
        vals.push(Value::Text(format!("%{m}%")));
        idx += 1;
    }
    if let Some(sc) = obj
        ...
        .and_then(|v| v.as_i64())
    {
        conds.push(format!("l.status_code = ?{idx}"));
        vals.push(Value::Integer(sc));
        idx += 1;
    }
    if let Some(sd) = obj
        .and_then(|o| o.get("startDate"))
        .and_then(|v| v.as_i64())
    {
        conds.push(format!("l.created_at >= ?{idx}"));
        vals.push(Value::Integer(sd));
        idx += 1;
    }
    if let Some(ed) = obj.and_then(|o| o.get("endDate")).and_then(|v| v.as_i64()) {
        conds.push(format!("l.created_at <= ?{idx}"));
        vals.push(Value::Integer(ed));
        idx += 1;
    }
    ...
    let total: i64 = conn
        .query_row(&cnt_sql, rusqlite::params_from_iter(vals.iter()), |r| {
            r.get(0)
        })
        .unwrap_or(0);
    ...
    let mut stmt = conn
        ...
        .map_err(|e| format!("Prepare: {e}"))?;
    let data = stmt
        .query_map(rusqlite::params_from_iter(vals.iter()), read_log_row)
        .map_err(|e| format!("Query: {e}"))?
        .filter_map(|r| r.ok())
        .collect();
